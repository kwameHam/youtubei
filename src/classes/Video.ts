...
export default class Video extends BaseVideo implements VideoAttributes {
	...
	load(data: YoutubeRawData): Video {
		...
		this.publishDate = data[2].playerResponse?.microformat?.playerMicroformatRenderer?.publishDate || null
		...
		const itemSectionRenderer = data[3].response.contents.twoColumnWatchNextResults.results.results.contents.find(
			(c: YoutubeRawData) => c.itemSectionRenderer && c.itemSectionRenderer.sectionIdentifier === 'comment-item-section'
		).itemSectionRenderer;
